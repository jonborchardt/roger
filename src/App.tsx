...
function respondToCommand(input: string, ctx: Context = {}, flags: SceneFlags = {}): MatchResult {
const raw = (input ?? '').trim();
if (!raw) {
return reply('system.empty', 'Say something. The bay is not going to narrate itself.');
}
...
function resolvePronouns(nouns: Set<string>, pronouns: Set<string>, ctx: Context): Set<string> {
...
'No. Absolutely not.',
waitA:
'You wait. The bay waits back. Neither of you improves.',
waitB:
'You pause. The hum continues. Whatever comes next is still your problem.',
jump:
'You jump. Gravity accepts this with a shrug. Nothing improves.',
kickAir:
'You kick at the air. The air wins by not caring.',
...
useTabMissing:
'You do not have anything latch-like to make that work.',
useNothing:
...
talkGeneral:
'You talk. The bay listens with all the empathy of scrap metal.',
...
action: (_p, _raw, ctx) => {
if (!ctx.global200) {
return { kind: 'pass', reason: 'QA disabled, allow other handlers', ruleId: 'qa.entry' };
}
return reply('qa.entry', 'QA is not wired to a menu here. Say "help" for commands.');
},
},
...
if (inv.size === 0) return reply('system.inventory', replies.inventoryEmpty);
const items = [...inv.values()].sort();
return reply('system.inventory', `${replies.inventoryPrefix} ${items.join(', ')}`);
},
},
{
id: 'system.status',
...
parts.push(flags.machineRepaired ? 'machine: latched' : 'machine: missing latch');
parts.push(`focus: ${ctx.lastFocus ?? 'none'}`);
return reply('system.status', `${replies.statusPrefix} ${parts.join(', ')}`);
},
},
...
id: 'scene.askWhere',
priority: 2850,
match: (p) => p.intents.has('ask') && (p.all.has('where') || p.pronouns.has('here')),
action: (_p, _raw, ctx) => {
ctx.lastFocus = 'area';
return reply('scene.askWhere', pickVariant(ctx, 'lookArea', [replies.lookAreaA, replies.lookAreaB]));
},
},
...
id: 'scene.lookAround',
priority: 2000,
match: (p) => p.intents.has('look') && (p.nouns.has('area') || p.preps.has('around') || p.pronouns.has('here')),
action: (_p, _raw, ctx) => {
ctx.lastFocus = 'area';
return reply('scene.lookAround', pickVariant(ctx, 'lookArea', [replies.lookAreaA, replies.lookAreaB]));
},
},
...
priority: 1988,
match: (p) => p.intents.has('look') && p.nouns.has('lights') && p.adjs.has('flicker'),
action: (_p, _raw, ctx, flags) => {
ctx.lastFocus = 'lights';
return reply(
scene.lookLightsFlicker,
flags.lightsFlickering ? replies.lookLightsFlicker : (flags.lightsWorking ? replies.lookLightsWorking : replies.lookLightsDead),
);
},
},
{
...
action: (_p, _raw, ctx, flags) => {
ctx.lastFocus = 'lights';
return reply('scene.lookLights', flags.lightsWorking ? replies.lookLightsWorking : replies.lookLightsDead);
},
},
{
...
if (!flags.wallHasStains) return reply('scene.lookStains', 'You do not see stains anymore. That is suspicious.');
...
if (!flags.wallHasSymbols) return reply('scene.lookSymbols', 'No markings worth calling symbols.');
...
id: 'scene.lookPanel',
priority: 1974,
match: (p) => p.intents.has('look') && (p.nouns.has('panel') || p.nouns.has('controls')),
action: (_p, _raw, ctx) => {
ctx.lastFocus = 'panel';
...
return reply('scene.lookRedDisc', 'You do not see the red disc anymore.');
}
...
if (!flags.hasBlueCrate) return reply('scene.lookBlueCrate', 'You do not see the blue box anymore.');
...
if (!flags.hasSilverCylinder && !(ctx.inventory?.has('silver cylinder') ?? false)) {
return reply('scene.lookSilverCylinder', 'You do not see the silver cylinder anymore.');
}
...
action: (_p, _raw, ctx, flags) => {
ctx.lastFocus = 'slime';
return reply('scene.lookSlime', flags.slimePresent ? replies.lookSlimePresent : replies.lookSlimeAbsent);
},
},
...
id: 'scene.smellOzone',
priority: 1890,
match: (p) => p.intents.has('smell') && (p.nouns.has('ozone') || p.all.has('electric') || p.all.has('burnt')),
action: (_p, _raw, ctx) => {
ctx.lastFocus = 'air';
...
return reply('scene.listen', flags.ambientHum ? replies.listenHum : replies.listenQuiet);
},
},
{
id: 'scene.touchSlime',
...
id: 'scene.touchPanel',
priority: 1885,
match: (p) => p.intents.has('touch') && (p.nouns.has('panel') || p.nouns.has('controls')),
action: (_p, _raw, ctx) => {
ctx.lastFocus = 'panel';
...
action: (_p, _raw, ctx) => reply('scene.wait', pickVariant(ctx, 'wait', [replies.waitA, replies.waitB])),
},
{
id: 'scene.jump',
priority: 1874,
...
id: 'scene.kickRubble',
priority: 1873,
match: (p) => p.intents.has('kick') && (p.nouns.has('rubble') || p.nouns.has('debris') || p.nouns.has('deck')),
...
id: 'scene.throwDisc',
priority: 1871,
match: (p) => p.intents.has('throw') && (p.nouns.has('redDisc') || p.all.has('disc') || p.all.has('ring')),
action: (_p, _raw, ctx, flags) => {
const hasDisc = (ctx.inventory?.has('red disc') ?? false) || (flags.hasRedDisc ?? false);
if (!hasDisc) return reply('scene.throwDisc', 'You cannot throw what you do not have.');
...
action: () => reply('scene.throwGeneral', 'You mime throwing something. It is not as satisfying as throwing something real.'),
},
...
priority: 1860,
match: (p) => p.intents.has('search') && (p.nouns.has('rubble') || p.nouns.has('deck') || p.nouns.has('debris')),
action: (_p, _raw, ctx) => {
ctx.lastFocus = 'rubble';
...
action: () => reply('scene.searchGeneral', 'You search. Pick a target: rubble, debris, machine, panel, archway.'),
},
{
id: 'scene.scanMachine',
priority: 1858,
match: (p) => p.intents.has('scan') && (p.nouns.has('machine') || p.all.has('left')),
action: (_p, _raw, ctx, flags) => {
ctx.lastFocus = 'machine';
if (!flags.leftMachinePowered) return reply('scene.scanMachine', 'The machine looks inert. No glow, no hum, no help.');
...
action: () => reply('scene.scanGeneral', 'You scan with your eyes because you do not have an actual scanner.'),
},
{
...
action: (_p, _raw, ctx, flags) => {
ctx.lastFocus = 'symbols';
if (!flags.wallHasSymbols) return reply('scene.readSymbols', 'There is nothing to read here.');
const k = ctx.knowledge ?? new Set<string>();
ctx.knowledge = k;
...
action: () => reply('scene.readGeneral', 'There is not much to read that wants to be understood. Try the symbols.'),
},
{
id: 'scene.pressPanel',
priority: 1854,
match: (p) => p.intents.has('press') && (p.nouns.has('panel') || p.nouns.has('controls')),
action: (_p, _raw, ctx, flags) => {
ctx.lastFocus = 'panel';
const hasCylinder = ctx.inventory?.has('silver cylinder') ?? false;
if (!flags.panelUnlocked && !hasCylinder) return reply('scene.pressPanel', replies.pressPanelDead);
...
action: () => reply('scene.pressGeneral', 'You press at random. The room does not reward that. Try the panel.'),
},
{
id: 'scene.cleanWall',
priority: 1852,
match: (p) => p.intents.has('clean') && (p.nouns.has('walls') || p.nouns.has('stains')),
action: (_p, _raw, ctx) => {
ctx.lastFocus = 'stains';
...
id: 'scene.cleanFloor',
priority: 1851,
match: (p) => p.intents.has('clean') && (p.nouns.has('deck') || p.nouns.has('rubble')),
action: (_p, _raw, ctx) => {
ctx.lastFocus = 'deck';
...
action: () => reply('scene.cleanGeneral', 'You attempt to clean. The bay offers you more work.'),
},
...
action: (_p, _raw, ctx, flags) => {
ctx.lastFocus = 'archway';
if (!flags.archwayOpen) return reply('scene.approachArchway', 'You approach. It is sealed. The bay wins.');
...
action: (_p, _raw, ctx, flags) => {
ctx.lastFocus = 'archway';
if (!flags.archwayOpen) return reply('scene.enterArchway', 'You try to go through. It is closed.');
if (flags.archwaySafe) return reply('scene.enterArchway', replies.enterArchwaySafe);
...
action: (_p, _raw, ctx, flags) => {
ctx.lastFocus = 'redDisc';
if (!flags.hasRedDisc) return reply('scene.takeRedDisc', 'You do not see a red disc to take.');
...
action: (_p, _raw, ctx, flags) => {
ctx.lastFocus = 'silverCylinder';
if (!flags.hasSilverCylinder) return reply('scene.takeSilverCylinder', 'You do not see the silver cylinder.');
...
if (ctx.inventory?.has('metal tab')) return reply('scene.takeMetalTab', 'You already have the metal tab.');
if (ctx.knowledge?.has('foundMetalTab')) {
ctx.inventory?.add('metal tab');
return reply('scene.takeMetalTab', 'You take the metal tab. It looks like a latch piece.');
}
return reply('scene.takeMetalTab', 'You do not see any metal tab to take.');
},
},
{
...
priority: 1800,
match: (p) => p.intents.has('use') && p.nouns.has('silverCylinder') && (p.nouns.has('panel') || p.nouns.has('controls')),
action: (_p, _raw, ctx, flags) => {
ctx.lastFocus = 'panel';
const hasCylinder = ctx.inventory?.has('silver cylinder') ?? false;
if (!hasCylinder) return reply('scene.useCylinderOnPanel', 'You do not have a silver cylinder.');
flags.panelUnlocked = true;
...
priority: 1799,
match: (p) => p.intents.has('use') && p.nouns.has('silverCylinder') && (p.nouns.has('machine') || p.all.has('left')),
action: (_p, _raw, ctx, flags) => {
ctx.lastFocus = 'machine';
const hasCylinder = ctx.inventory?.has('silver cylinder') ?? false;
if (!hasCylinder) return reply('scene.useCylinderOnMachine', 'You do not have a silver cylinder.');
if (!flags.leftMachinePowered) return reply('scene.useCylinderOnMachine', 'The machine is not powered. The coupling is dead.');
if (!ctx.inventory?.has('metal tab')) return reply('scene.useCylinderOnMachine', replies.useCylinderOnMachineFail);
// If they have the tab, guide them to use it explicitly (deterministic gating).
return reply('scene.useCylinderOnMachine', 'The cylinder wants a latch. Try using the metal tab on the machine.');
},
},
{
id: 'scene.useTabOnMachine',
priority: 1798,
match: (p) => p.intents.has('use') && p.nouns.has('metalTab') && p.nouns.has('machine'),
action: (_p, _raw, ctx, flags) => {
ctx.lastFocus = 'machine';
if (!ctx.inventory?.has('metal tab')) return reply('scene.useTabOnMachine', replies.useTabMissing);
const hasCylinder = ctx.inventory?.has('silver cylinder') ?? false;
if (!hasCylinder) return reply('scene.useTabOnMachine', 'You latch the coupling, but there is no cylinder to seat.');
...
if (ctx.lastFocus) {
return reply('scene.useGeneral', `Use ${renderTarget(ctx.lastFocus)} how? Try "use X on Y".`);
}
return reply('scene.useGeneral', `You try "${raw}". Nothing here is cooperating.`);
},
},
...
p.person.has('roger') &&
(p.intents.has('ask') || p.question) &&
(p.all.has('how') || p.all.has('survive') || p.all.has('alive') || p.all.has('why')),
action: (_p, _raw, ctx) => {
ctx.lastFocus = 'roger';
...
p.person.has('roger') &&
(p.intents.has('ask') || p.question) &&
(p.all.has('abduct') || p.all.has('base') || p.all.has('life') || p.all.has('support')),
action: (_p, _raw, ctx) => {
ctx.lastFocus = 'roger';
...
(p.all.has('feel') || p.all.has('scared') || p.all.has('afraid') || p.all.has('brave')),
action: (_p, _raw, ctx) => {
ctx.lastFocus = 'roger';
...
function fallbackReply(p: Parse, raw: string, ctx: Context, flags: SceneFlags): MatchResult {
...
if (intent === 'search') return reply('fallback.search', 'Search what? Try rubble, debris, machine, or panel.');
if (intent === 'scan') return reply('fallback.scan', 'Scan what? Try the machine or the symbols.');
if (intent === 'read') return reply('fallback.read', 'Read what? Try the symbols.');
if (intent === 'press') return reply('fallback.press', 'Press what? Try the panel.');
if (intent === 'clean') return reply('fallback.clean', 'Clean what? Try stains or the floor.');
if (intent === 'approach') return reply('fallback.approach', 'Approach what? Try the archway or the machine.');
if (intent === 'enter') return reply('fallback.enter', 'Enter what? The archway is the only obvious path.');
if (intent === 'smell') return reply('fallback.smell', replies.smellMetal);
if (intent === 'listen') return reply('fallback.listen', flags.ambientHum ? replies.listenHum : replies.listenQuiet);
if (intent === 'touch') return reply('fallback.touch', replies.touchColdMetal);
...
if (intent === 'throw') return reply('fallback.throw', 'Throw what? The red disc is a solid candidate.');
if (intent === 'kick') return reply('fallback.kick', 'Kick what? Try rubble. It will not thank you.');
if (intent === 'jump') return reply('fallback.jump', replies.jump);
if (intent === 'wait') return reply('fallback.wait', pickVariant(ctx, 'wait', [replies.waitA, replies.waitB]));
...
// Make it fill the renderer; call this in updateLayout too
  ...
  bg.position.set(0, 0);
      ...
      const title = new Text({
        text: 'Voice Command Scene (Deterministic)',
        style: {
          fill: '#ffffff',
          fontSize: 30,
